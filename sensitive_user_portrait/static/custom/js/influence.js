...
function draw_rank_table(data){
    ...
    if(data.length == 0){
        ...
        html += '<th style="text-align:center;min-width:30px;">重要度</th>';
        html += '<th style="text-align:center;">活跃度</th>';
        html += '<th style="text-align:center;">影响力</th>';
        html += '<th style="text-align:center;min-width:30px;">敏感度</th></thead>';
        for(var i=0;i<data.length;i++){
...
$('#range_choose').change(function(){
    ...
    if($('#range_choose').val() == 'in_nolimit') {
        ...
        sort_select += '<option value="ses">敏感度</option>';
...
function draw_in_key_rank_table(data){
    ...
    if(data.length == 0){
        ...
        var scope_dict ={'all_limit_keyword':'全网-按关键词','in_limit_keyword':'库内-按关键词','in_limit_hashtag':'库内-按微话题'}
                var norm_dict ={'weibo_num': '微博数','fans': '粉丝数','bci': '影响力','bci_change':'突发影响力变动','ses':'敏感度','ses_change':'突发敏感度变动','imp':'重要度','imp_change':'突发重要度变动','act':'活跃度','act_change':'突发活跃度变动'}
