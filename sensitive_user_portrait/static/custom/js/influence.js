...
function submit_rank(){
	...
	var number = $("input[name='sort_num_range']:checked").val();
	//全网实时
	if(sort_scope=='in_intime'){
		...
		if(day_select*6 > hh){
			...
			var url = '/user_rank/temporal_rank/?task_type='+day_select+'&sort='+sort_norm+'&number='+number;
	...
	//console.log(keyword);
		if(keyword == ''){  //检查输入词是否为空
			...
			if(keyword == undefined){  //没有输入的时候，更新表格及文字
				...
				var url = '/user_rank/user_sort/?username='+username+'&time='+day_select+'&sort_norm='+sort_norm+'&sort_scope='+sort_scope+'&number='+number;
				$('#rec_range').empty();
				$('#rec_detail').empty();
				$('#rec_rank_by').empty();
				$('#rec_time_range').empty();
				...
				var url = '/user_rank/user_sort/?time=-1&username='+username+'&st='+time_from +'&et='+time_to+'&sort_norm='+sort_norm+'&sort_scope='+sort_scope+'&arg='+keyword+'&number='+number;
				console.log(url);
