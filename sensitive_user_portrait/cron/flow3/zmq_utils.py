...
def send_weibo(sender,poller, controller, total_count=0, total_cost=0):
    ...
    for each in file_list:
        ...
        if 'bin' in each and 'ok' not in each:
            ...
            if '%s.bin.ok' % filename in file_set and '%s_yes3.txt' % filename not in txt_list:
                ...
                with open(os.path.join(WRITTEN_TXT_PATH, '%s_yes3.txt' % filename), 'w') as fw:
