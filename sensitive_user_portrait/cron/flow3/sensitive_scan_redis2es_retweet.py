...
reload(sys)
sys.path.append('../../')
from global_utils import es_retweet as es
from global_utils import sensitive_retweet_redis_dict, sensitive_comment_redis_dict, sensitive_redis_host_list
from global_utils import sensitive_retweet_index_name_pre, sensitive_be_retweet_index_name_pre
from global_config import R_BEGIN_TIME
from parameter import DAY, RUN_TYPE, RUN_TEST_TIME
from time_utils import ts2datetime, datetime2ts
from sensitive_retweet_mappings import retweet_es_mappings, be_retweet_es_mappings
...
def get_db_num(timestamp):
    ...
    db_number = (((date_ts - begin_ts) / (DAY * 7))) % 2 +3
    #run_type
    if RUN_TYPE == 0:
        db_number = 3
...
def scan_retweet():
    ...
    #get redis db
    retweet_redis = sensitive_retweet_redis_dict[str(db_number)]

    # 1. 判断即将切换的db中是否有数据
    print sensitive_redis_host_list, str(db_number)
    if RUN_TYPE:
    ...
    while True:
        ...
        if retweet_bulk_action:
            es.bulk(retweet_bulk_action, index=sensitive_retweet_index_name_pre+str(db_number), doc_type='user')
        if be_retweet_bulk_action:
            es.bulk(be_retweet_bulk_action, index=sensitive_be_retweet_index_name_pre+str(db_number), doc_type='user')
        retweet_bulk_action = []
        be_retweet_bulk_action = []
        end_ts = time.time()
        #run_type
