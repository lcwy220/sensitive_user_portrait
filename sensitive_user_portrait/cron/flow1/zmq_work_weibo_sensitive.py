...
def cal_propage_work(item, sensitive_words):
    ...
    if message_type == 1:
        ...
        if sensitive_result:
            cluster_redis.sadd("s_" + user + '_origin_weibo', mid)
        else:
        ...
        if not sensitive_result:
            ...
            cluster_redis.sadd('s_' + user + '_comment_weibo', retweeted_mid) # comment on sensitive weibo
