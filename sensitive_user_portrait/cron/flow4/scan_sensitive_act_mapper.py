...
from parameter import RUN_TYPE, DAY, pre_act,sen_pre_act
...
def scan_mapper(pre, sen_pre, r):
    ...
    while 1:
        re_scan = r.hscan(sen_hash_name, cursor, count=1000)
        cursor = re_scan[0]
        ip_dict = re_scan[1]
        uid_list = ip_dict.keys()
        if uid_list:
            r.lpush('sensitive_act_uid_list', json.dumps(uid_list))
            count += len(uid_list)
            ts = time.time()
...
if __name__ == "__main__":
    scan_mapper(pre_act, sen_pre_act, redis_activity)
