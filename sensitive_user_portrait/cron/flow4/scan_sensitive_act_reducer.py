...
def scan_reducer():
    ...
    index_name = sen_act_index_pre + date
    #sen_index_name = sen_ip_index_pre + date
    mapping(index_name, sen_act_doc_type)
    count = 0
    bulk_action = []
    tb = time.time()

    while 1:
        tmp_list = redis_activity.rpop('sensitive_act_uid_list')
        if tmp_list:
            ...
            for i in range(len(uid_list)):
                ...
                save_dict['sensitive_activity_dict'] = ip_dict[i]
                bulk_action.extend([{'index':{'_id':uid}}, save_dict])
            es_cluster.bulk(bulk_action, index=index_name, doc_type=sen_act_doc_type)
            bulk_action = []
            count += len(uid_list)
            te = time.time()
            if RUN_TYPE == 0:
